...
<head>
    ...
    <script type="text/javascript">
        ...
        function getSignDetail(id){
            window.open ("<c:url value="/productDetail.do"/>?product_id="+id,"标的审核","height=650,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
        }
    </script>
</head>
<body>
    ...
    <div style="padding-bottom: 0px; padding-left: 10px; padding-right: 10px; padding-top: 15px">
        <div>
            <table border=0 cellspacing=0 cellpadding=0 width="100%">
                <tbody>
                <tr>
                    <td class=main_alll_h2 height=28 width=120><a href="#">标的查询--待发布</a></td>
                    <td width=2>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                </tbody>
            ...
            <div style="padding-bottom: 10px; background-color: #fff; padding-left: 10px; width: 1120px; padding-right: 10px; padding-top: 10px">
                <form id="form1" name="form1" action="<c:url value="/signQuery.do"/>" method="post">
                ...
                <div>
                    <table id="listTable" style="width: 100%; color: #333333;border-collapse: collapse;"
                        ...
                        <c:if test="${empty signList}">
                            <tr class=gvItem>
                                <td colspan="12" align="center">查询数据为空！</td>
                            </tr>
                        </c:if>
                        <c:if test="${not empty signList}">
                            <c:forEach items="${signList}" var="list">
                                <tr class=gvItem>
                                    ...
                                    <%--<td align="center">&nbsp;<c:if test="${list.loan_type==0}">个人借款</c:if>--%>
                                    ...
                                    <td><a href="javascript:getSignDetail('<c:out value='${list.product_id}'/>')">发布审核</a></td>
                                </tr>
                            </c:forEach>
                        </c:if>
                        </tbody>
