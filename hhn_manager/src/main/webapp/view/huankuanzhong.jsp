...
<head>
    ...
    <script type="text/javascript">
        ...
        function aheahMoney(id){
            window.open ("<c:url value="/aheadMoneyDetail.do"/>?product_id="+id,"提前还款","height=650,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
        }

        function backMoney(id){
            window.open ("<c:url value="/backMoneyDetail.do"/>?product_id="+id,"还款明细","height=650,width=800,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
        }
    </script>
</head>
<body>
    ...
    <div style="padding-bottom: 0px; padding-left: 10px; padding-right: 10px; padding-top: 15px">
        <div>
            <table border=0 cellspacing=0 cellpadding=0 width="100%">
                <tbody>
                <tr>
                    <td class=main_alll_h2 height=28 width=120><a href="#">标的查询--还款中</a></td>
                    <td width=2>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                </tbody>
            ...
            <div style="padding-bottom: 10px; background-color: #fff; padding-left: 10px; width: 1120px; padding-right: 10px; padding-top: 10px">
                <form id="form1" name="form1" action="<c:url value="/backMoney.do"/>" method="post">
                ...
                <div>
                    <table id="listTable" style="width: 100%; color: #333333;border-collapse: collapse;"
                        ...
                        <c:if test="${empty productList}">
                            <tr class=gvItem>
                                <td colspan="15" align="center">查询数据为空！</td>
                            </tr>
                        </c:if>
                        <c:if test="${not empty productList}">
                            <c:forEach items="${productList}" var="list">
                                <tr class=gvItem>
                                    ...
                                    <td align="center"><f:formatDate value="${list.nextTime}" pattern="yyyy-MM-dd"/></td>
                                    <td align="center"><c:out value="${list.nextAmount}"/></td>
                                    <td align="center">回款中</td>
                                    <td><a href="javascript:aheahMoney('<c:out value='${list.product_id}'/>')">提前结清</a>
                                         | <a href="javascript:backMoney('<c:out value='${list.product_id}'/>')">还款</a></td>
                                </tr>
                            </c:forEach>
                        </c:if>
                        </tbody>
